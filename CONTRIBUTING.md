...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (contributing to firstmate requires **no-mistakes v1.46.0+** for structured attestation; without a fork, plain `no-mistakes init` still works for maintainers with push access).
   Inside a firstmate operational home, `bin/fm-fork-target.sh init <dir>` applies the local `config/fork-url` or `config/fork-owner` declaration for you and is the repair path for a clone already initialized against an unwritable target; [docs/configuration.md](docs/configuration.md) owns those settings.
