...
AUTHORIZED_REPOS=
if [ "$RELAUNCH" -eq 1 ]; then
  AUTHORIZED_REPOS=$(grep '^authorized_repo=' "$RELAUNCH_META" 2>/dev/null | cut -d= -f2- || true)
else
  AUTHORIZED_REPOS=$(fm_pr_project_authorized_repositories "$PROJ_ABS" 2>/dev/null || true)
fi
...
}
{
  ...
  while IFS= read -r authorized_repo; do
    ...
    echo "authorized_repo=$authorized_repo"
  done <<< "$AUTHORIZED_REPOS"
