...
# fm_merge_outcome_report <home> <state> <task-id> <pr-url> <origin> [<outcome>] [<authority>]
#
...
fm_merge_outcome_report() {  # <home> <state> <task-id> <pr-url> <origin> [<outcome>] [<authority>]
  ...
  # Merge authority describes who authorized a MERGE, so it is meaningless on a
  # close and an incoherent request rather than something to ignore quietly.
  [ "$outcome" = merged ] || [ -z "$suffix" ] || return 2
  ...
  if [ "$outcome" = merged ]; then
    wake_note="check: merge landed: $id $FM_PR_URL$suffix"
