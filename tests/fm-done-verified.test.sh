...
# RULE ONE's third condition on its own. A run can be terminal AND not failed and
# still have accepted nothing: `completed` says the pipeline stopped, never that
# it approved what it read. Only an accepted outcome may verify, so a status that
# reports stopping without approving has to be refused - and refused by the
# OUTCOME gate rather than by the still-running one, which is why the reason is
# asserted and not only the code.
test_a_terminal_run_that_accepted_nothing_does_not_verify() {
  local dir shipped result
  dir=$(make_world run-completed-unaccepted)
  shipped=$(git -C "$dir/wt" rev-parse HEAD)
  printf 'done: pr=https://github.com/o/r/pull/7 head=%s - shipped\n' "$shipped" \
    > "$dir/state/task-v.status"

  # Terminal, not failed, and naming no accepted outcome.
  result=$(FAKE_GH_OUT="OPEN	$shipped	fm/task-v	SUCCESS" \
    FAKE_NM_STATUS="branch: fm/task-v
head: $shipped
status: completed" verify "$dir")
  [ "${result%%$'\t'*}" = 3 ] \
    || fail "a completed run that accepted nothing verified: $result"
  case "$result" in
    *"does not establish that it accepted this work"*) ;;
    *) fail "a bare completed run was refused by the wrong gate: $result" ;;
  esac

  # The other shape of the same mistake: the run names an outcome, and that
  # outcome is `completed`. Terminal and not failed, and still not acceptance.
  result=$(FAKE_GH_OUT="OPEN	$shipped	fm/task-v	SUCCESS" \
    FAKE_NM_STATUS="branch: fm/task-v
head: $shipped
status: completed
outcome: completed" verify "$dir")
  [ "${result%%$'\t'*}" = 3 ] \
    || fail "a run whose outcome is merely 'completed' verified: $result"
  case "$result" in
    *"does not establish that it accepted this work"*) ;;
    *) fail "an outcome of 'completed' was refused by the wrong gate: $result" ;;
  esac

  # The positive control: the same terminal status once it names an accepted
  # outcome, so the assertions above cannot pass vacuously.
  result=$(FAKE_GH_OUT="OPEN	$shipped	fm/task-v	SUCCESS" \
    FAKE_NM_STATUS="branch: fm/task-v
head: $shipped
status: completed
outcome: checks-passed" verify "$dir")
  [ "${result%%$'\t'*}" = 0 ] \
    || fail "an accepted checks-passed run stopped verifying: $result"
  pass "a terminal run that accepted nothing does not verify"
}
...
# Merge authority and terminal outcome are two independent optional arguments on
# one emitter, so combining them is exactly where one could quietly displace the
# other. Authority still tags the merged ledger line and its wake; on a close it
# describes nothing that happened, so it is refused as an incoherent request
# rather than dropped in silence, which is the direction every other gate here
# takes when two facts disagree.
test_authority_rides_a_merge_and_is_refused_on_a_close() {
  local claim dir queue rc
  claim="done: pr=$PR_A head=$HEAD_A - shipped"
  dir=$(make_claim_world authority-on-merge "$claim") \
    || fail "the merge-authority fixture failed"
  fm_merge_outcome_report "$dir" "$dir/state" task-v "$PR_A" poll merged yolo \
    || fail "a merge carrying its authority could not be published"
  queue=$(cat "$dir/state/.wake-queue" 2>/dev/null || true)
  case "$queue" in
    *"merge landed"*yolo*) ;;
    *) fail "the merged outcome dropped the authority that authorised it: $queue" ;;
  esac
  dir=$(make_claim_world authority-on-close "$claim") \
    || fail "the close-authority fixture failed"
  rc=0
  fm_merge_outcome_report "$dir" "$dir/state" task-v "$PR_A" poll closed-unmerged yolo \
    || rc=$?
  [ "$rc" -eq 2 ] \
    || fail "an authority on a close was accepted (rc=$rc) instead of refused as incoherent"
  assert_absent "$dir/state/task-v.done-verdict" \
    "a refused outcome request still wrote a verdict"
  assert_absent "$dir/state/.wake-queue" \
    "a refused outcome request still published a wake"
