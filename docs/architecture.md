...
Because that rule reports the names of checks that are not green, a pull request with no checks at all would yield no names and read as green, so the rollup's entry count is a separate pre-merge condition and an empty rollup refuses.
...
The no-checks refusal is waived through the same flag but only by a parenthesised sentinel name that no check can carry, so waiving one red check never also waives a pull request no check ever ran against.
