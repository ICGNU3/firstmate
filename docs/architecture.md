...
A concurrent replacement remains armed, every non-terminal or invalid observation remains unchanged, and retirement never performs task or persistent-secondmate cleanup.
...
In each home the scan considers only that home's long-inactive direct ordinary crewmates, excludes captain-held work, and accepts only `done`, `done-unverified`, `failed`, or the pre-validation `ready` handoff from `bin/fm-crew-state.sh`; an unestablished claim is still an undelivered outcome, and a stopped worker waiting on firstmate is still nobody's action, so neither may fall out of the backstop, and the `ready` case is reported as the handoff it is rather than as a terminal outcome.
...
That section also surfaces a line carrying no recognised verb, marked UNRECOGNISED so it is not read as a state it never claimed, under a per-task cap (`FM_DRAIN_UNRECOGNISED_MAX`) so one chatty worker cannot bury the rest of the drain; a held-back line freezes that task's presentation cursor at the last line actually shown, so the remainder re-presents on the next drain rather than being acknowledged unseen.
