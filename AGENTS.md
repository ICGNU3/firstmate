...
config/fork-owner  optional one-token forge account this home owns its forks under, used to initialize the no-mistakes gate against a push target this home can actually write; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Push target for the no-mistakes gate"
