...
config/fork-url  optional complete push URL used verbatim to initialize the no-mistakes gate (https/http/ssh/git+ssh/file or user@host:path); LOCAL, gitignored; inherited by secondmate homes; takes precedence over config/fork-owner; invalid values fail closed; see docs/configuration.md "Push target for the no-mistakes gate"
config/fork-owner  optional one-token forge account this home owns its forks under, used to assemble a push target from each clone's origin; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Push target for the no-mistakes gate"
